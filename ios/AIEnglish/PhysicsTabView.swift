...
private struct PhysicsStructureFile: Codable {
    let version: Int?
    let title: String
    let subtitle: String?
    let badge: String?
    let sections: [PhysicsSection]
}

private struct PhysicsSection: Codable {
    let title: String
    let blocks: [PhysicsBlock]
}

private struct PhysicsBlock: Codable {
    let type: String
    let text: String?
    let title: String?
    let headers: [String]?
...
/// 北京中考物理试卷结构（physics_beijing_structure.json）
struct PhysicsTabView: View {
    @Environment(\.fontScale) private var fontScale
    @State private var doc: PhysicsStructureFile?
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        ForEach(Array(file.sections.enumerated()), id: \.offset) { _, sec in
                            PhysicsSectionCard(section: sec, fontScale: fontScale)
                        }
                    }
    ...
    private func load() async {
        guard let url = Bundle.main.url(forResource: "physics_beijing_structure", withExtension: "json")
                ?? Bundle.main.url(forResource: "physics_beijing_structure", withExtension: "json", subdirectory: "web/data") else {
            loadError = "缺少 physics_beijing_structure.json"
            return
        }
        do {
            let data = try Data(contentsOf: url)
            doc = try JSONDecoder().decode(PhysicsStructureFile.self, from: data)
        } catch {
...
private struct PhysicsSectionCard: View {
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 10) {
                ForEach(Array(section.blocks.enumerated()), id: \.offset) { _, block in
                    PhysicsBlockView(block: block, fontScale: fontScale)
                }
            }
        }
        .frame(maxWidth: .infinity, alignment: .leading)
...
private struct PhysicsBlockView: View {
    ...
    var body: some View {
        Group {
            switch block.type {
            case "table":
                PhysicsTableView(headers: block.headers ?? [], rows: block.rows ?? [], fontScale: fontScale)
            case "subheading":
                Text(block.text ?? "")
...
private struct PhysicsTableView: View {
    let headers: [String]
    let rows: [[String]]
    let fontScale: FontScale
