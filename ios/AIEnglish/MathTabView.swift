...
private struct MathStructureFile: Codable {
    let version: Int?
    let title: String
    let subtitle: String?
    let badge: String?
    let sections: [MathSection]
}

private struct MathSection: Codable {
    let title: String
    let blocks: [MathBlock]
}

private struct MathBlock: Codable {
    let type: String
    let text: String?
    let title: String?
    let headers: [String]?
...
/// 北京中考数学试卷结构（math_beijing_structure.json）
struct MathTabView: View {
    @Environment(\.fontScale) private var fontScale
    @State private var doc: MathStructureFile?
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        ForEach(Array(file.sections.enumerated()), id: \.offset) { _, sec in
                            MathSectionCard(section: sec, fontScale: fontScale)
                        }
                    }
    ...
    private func load() async {
        guard let url = Bundle.main.url(forResource: "math_beijing_structure", withExtension: "json")
                ?? Bundle.main.url(forResource: "math_beijing_structure", withExtension: "json", subdirectory: "web/data") else {
            loadError = "缺少 math_beijing_structure.json"
            return
        }
        do {
            let data = try Data(contentsOf: url)
            doc = try JSONDecoder().decode(MathStructureFile.self, from: data)
        } catch {
...
private struct MathSectionCard: View {
    let section: MathSection
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 10) {
                ForEach(Array(section.blocks.enumerated()), id: \.offset) { _, block in
                    MathBlockView(block: block, fontScale: fontScale)
                }
            }
        }
        .frame(maxWidth: .infinity, alignment: .leading)
...
private struct MathBlockView: View {
    ...
    var body: some View {
        Group {
            switch block.type {
            case "table":
                MathTableView(headers: block.headers ?? [], rows: block.rows ?? [], fontScale: fontScale)
            case "subheading":
                Text(block.text ?? "")
...
private struct MathTableView: View {
    let headers: [String]
    let rows: [[String]]
    let fontScale: FontScale
